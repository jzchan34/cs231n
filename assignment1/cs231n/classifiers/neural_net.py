...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    # shape (N, C).                                                             #
    #############################################################################
    f = lambda x : np.maximum(x, np.zeros(x.shape))
    h0 = np.dot(X, W1) + b1
    h1 = f(h0)
    ...
    grad = softmax #(N,C)
    grad[range(grad.shape[0]), y] -= 1 #(N,C)
  ...
  def train(self, X, y, X_val, y_val,
    ...
    for it in xrange(num_iters):
      ...
      #########################################################################
      data_batch = np.random.choice(num_train, batch_size, replace=True)
      X_batch = X[data_batch]
      y_batch = y[data_batch]
      #########################################################################
      #                             END OF YOUR CODE                          #
      #########################################################################
